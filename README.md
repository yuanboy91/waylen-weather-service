...
A lightweight Spring Boot service for querying current weather by **city name**, **ZIP / postal code**, **geographic coordinates**, or **OpenWeatherMap city ID**. It sits on top of the [OpenWeatherMap Current Weather API](https://openweathermap.org/current) and is designed to run inside a **private network reachable only via VPN**.
...
- **Four query modes**: city name / ZIP+country / lat-lon / city ID (dedicated REST endpoints)
...
- **Rate limiting**: Guava `RateLimiter` on `/api/weather/**` (tunable via `app.rate-limit.per-second`), rejected requests map to `429 TOO_MANY_REQUESTS`
...
└── src/
    ...
    └── test/java/com/waylen/weather/
        ...
        ├── client/WeatherServiceCacheTest.java        cache hits / isolation
...
| Parameter | Type | Required | Constraints |
|---|---|---|---|
| `id` | string | yes | 1–10 digits (OpenWeatherMap city ID, e.g. `5909629`) |

City IDs are globally unique in the OpenWeatherMap database, so this endpoint is **immune to duplicate city names**. IDs can be obtained from the official city list or from the `id` field of any weather response.
...
# 4. By OpenWeatherMap city ID (unique — resolves duplicate city names)
curl -s 'http://localhost:8099/api/weather/id?id=5909629'
```
...
Single-page UI (`static/index.html`) with four query tabs.
...
- **City ID discovery**: the `/id` endpoint requires callers to already know the ID; a geocoding-based city search endpoint could be added later to resolve names to IDs
